<!-- Header -->
<?php include_once("template/header.php") ?>
<!-- Body Part -->

    <!-- Banner -->

    <div class="wrapper">
    	<img class="banner_logo" src="images/banner-logo.svg" alt="">
    </div>

    <!-- Moto -->

    <div class="wrapper mb-50 mt-50">
      <div class="container align-center">
        <h2>શ્રી કર્ણાવત પરિવાર,</h2>
        <h3>શ્રી ઉકાણી, ભોજાણી, ખીમાણી, લાખાણી પરિવાર.</h3>
        <br>
        <br>
        <h2>ઉદ્દેશ : </h2>
        <h3>કર્ણાવત પરિવારમાં કુટુંબ ભાવના પ્રબળ બને તથા પરિવાર ના સભ્યો નો શૈક્ષણિક તથા આર્થિક વિકાશ થાય એ ઉદ્દેશ થી આ વેબસાઈટ નું નિર્માણ કરવામાં આવ્યુ છે.</h3>
      </div>
    </div>

<!-- Body Part -->
<!-- Footer -->
<?php include_once("template/footer.php") ?>
